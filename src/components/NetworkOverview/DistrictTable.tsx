...
export function DistrictTable({ districts, onDistrictClick }: DistrictTableProps) {
  const getDistrictStats = (villages: Village[]) => {
    ...
    const status = faults > 0 ? 'FAULT' : degraded > 0 ? 'DEGRADED' : 'HEALTHY';
    return { healthy, degraded, faults, status };
  };

  const getRowColor = (status: string) => {
    if (status === 'FAULT') return 'bg-fault bg-opacity-10 border-fault';
    if (status === 'DEGRADED') return 'bg-warning bg-opacity-10 border-warning';
        ...
        <table className="w-full font-mono text-sm">
          ...
          <tbody>
            {districts.map((district) => {
              const stats = getDistrictStats(district.villages);
              return (
                <tr
                  ...
                  className={`border-b border-border cursor-pointer hover:bg-cardInner transition-colors ${
                    stats.status === 'FAULT' ? 'animate-pulse' : ''
                  }`}
                >
